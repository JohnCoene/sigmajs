...
# ----------------------------- GET STARTED
nodes_demo <- sg_make_nodes(5)
edges_demo <- sg_make_edges(nodes_demo)
...
# ----------------------------- DYNAMIC
edges_demo$delay <- runif(nrow(edges_demo), 500, 2000) # between .5 and 2 seconds
...
# ----------------------------- ANIMATE
n <- nrow(nodes_demo)
nodes_demo$to_x <- runif(n, 5, 10)
nodes_demo$to_y <- runif(n, 5, 10)
...
animate <- sigmajs(height = 150) %>%
  ...
  sg_animate(mapping = list(x = "to_x", y = "to_y", size = "to_size", color = "to_color"),
             delay = 7000) %>%
  sg_settings(animationsTime = 5000)
