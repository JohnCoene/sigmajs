...
# 	           delay = 1000) %>%
# 	sg_settings(animationsTime = 3000,
# 	            mouseEnabled = FALSE,
# 	            touchEnabled = FALSE) -> net

# ----------------------------- GET STARTED
nodes_demo <- sg_make_nodes(7)
edges_demo <- sg_make_edges(nodes_demo, 10)
...
# ----------------------------- DYNAMIC
edges_demo$delay <- runif(nrow(edges_demo), 500, 3500) # between .5 and 2 seconds
...
animate <- sigmajs(height = 150) %>%
  ...
  sg_animate(mapping = list(x = "to_x", y = "to_y", size = "to_size", color = "to_color"),
             delay = 3400) %>%
  sg_settings(animationsTime = 7000,
              minNodeSize = 1,
              maxNodeSize = 5,
              mouseEnabled = FALSE,
	            touchEnabled = FALSE)
