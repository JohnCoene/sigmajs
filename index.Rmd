...
COLORS <- c("#D8482D", "#B30000", "#BB100A", "#B30000")
nodes$to_color <- sample(COLORS, 10, replace = TRUE)
...
Most functions have corresponding demos, execute the run command to see sigmajs in action.
...
for(i in 1:length(demos)){
	row <- data.frame(
		name = paste0("`", demos[[i]][1], "`"), 
		description = demos[[i]][2],
		run = paste0("`demo(", demos[[i]][1], ", package='sigmajs')`")
	)
