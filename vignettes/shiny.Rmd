...
Another good example of the package in the wild is [chirp.sh](https://chirp.sh).
...
All the [events](https://github.com/jacomyal/sigma.js/wiki/Events-API) are integrated in the package. In version `0.1.3` and prior all events are sent to the server by default, from `0.1.4` (currently dev version) onwards users have to specify which event they want to capture with `sg_events`.
